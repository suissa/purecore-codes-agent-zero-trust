...
import {
  A2AJsonRpcServer,
  A2AJsonRpcClient,
} from "../domains/a2a/bindings/json-rpc";
import {
  A2AHttpRestServer,
  A2AHttpRestClient,
} from "../domains/a2a/bindings/http-rest";
import { CertificateAuthority } from "../docs/mtls-agents";
...
async function demonstrateUltraSecureA2A() {
  ...
  const message = Message.text(
    "user",
    "Hello from Alpha to Beta via ultra-secure A2A!"
  );
  ...
  if ("status" in result) {
    ...
    if (completedData.messages.length > 1) {
      const lastMessage =
        completedData.messages[completedData.messages.length - 1];
  ...
  const streamingRequest = SendMessageRequest.make({
    message: streamingMessage,
  });

  console.log("📤 Iniciando stream...");
  for await (const event of channelBeta.sendStreamingMessage(
    streamingRequest
  )) {
  ...
  try {
    ...
    const socketAlphaToBeta = await channelAlpha.connectToPeer(
      "localhost",
      8446,
      "agent-beta"
    );
    const socketBetaToAlpha = await channelBeta.connectToPeer(
      "localhost",
      8445,
      "agent-alpha"
    );
    ...
    await channelAlpha.sendSecureMessage(
      "agent-beta",
      "Ultra-secure message from Alpha!",
      socketAlphaToBeta
    );
    await channelBeta.sendSecureMessage(
      "agent-alpha",
      "Ultra-secure response from Beta!",
      socketBetaToAlpha
    );
  ...
  const jsonRpcResponse = await jsonRpcServer.handleHttpRequest(
  ...
  for (const task of tasksList.tasks) {
    console.log(
      `  - Task ${task.id}: ${task.status} (${task.messages.length} mensagens)`
    );
  }
